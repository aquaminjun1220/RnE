# python note/cIRM_STFT_test.py --clean_file ./data/clean_data/TIMIT/TRAIN/SA1.WAV.wav --noise_file ./data/noise_data/DEMAND/DKITCHEN_ch01.wav --output_mixed_file D:\RnE\data\mixed_data\TRAIN/SA1.WAV.wav+DKITCHEN_ch01.wav--snr10.wav --output_re_file ./note/reconstructed/AA.wav --plot True
...
if __name__=='__main__':
    ...
    if args.plot:
        ...
        plt.subplot(421)
        librosa.display.specshow(clean_spec_r,sr=16000,hop_length=512, cmap="plasma")
        plt.xlabel("Time")
        plt.ylabel("Frequency")
        plt.colorbar(format='%+2.0f dB')
        plt.title("Clean Data Spectrogram (Real)")
        plt.subplot(422)
        librosa.display.specshow(clean_spec_i,sr=16000,hop_length=512, cmap="plasma")
        plt.xlabel("Time")
        plt.ylabel("Frequency")
        plt.colorbar(format='%+2.0f dB')
        plt.title("Clean Data Spectrogram (Imag)")
        plt.subplot(423)
        librosa.display.specshow(cIRM_spec_r,sr=16000,hop_length=512, cmap="plasma")
        plt.xlabel("Time")
        plt.ylabel("Frequency")
        plt.colorbar(format='%+2.0f dB')
        plt.title("cIRM Spectrogram (Real)")
        plt.subplot(424)
        librosa.display.specshow(cIRM_spec_i,sr=16000,hop_length=512, cmap="plasma")
        plt.xlabel("Time")
        plt.ylabel("Frequency")
        plt.colorbar(format='%+2.0f dB')
        plt.title("cIRM Spectrogram (Imag)")
        plt.subplot(425)
        librosa.display.specshow(mixed_spec_r,sr=16000,hop_length=512, cmap="plasma")
        plt.xlabel("Time")
        plt.ylabel("Frequency")
        plt.colorbar(format='%+2.0f dB')
        plt.title("Mixed Data Spectrogram (Real)")
        plt.subplot(426)
        librosa.display.specshow(mixed_spec_i,sr=16000,hop_length=512, cmap="plasma")
        plt.xlabel("Time")
        plt.ylabel("Frequency")
        plt.colorbar(format='%+2.0f dB')
        plt.title("Mixed Data Spectrogram (Imag)")
        plt.subplot(427)
        librosa.display.specshow(re_spec_r,sr=16000,hop_length=512, cmap="plasma")
        plt.xlabel("Time")
        plt.ylabel("Frequency")
        plt.colorbar(format='%+2.0f dB')
        plt.title("Re Data Spectrogram (Real)")
        plt.subplot(428)
        librosa.display.specshow(re_spec_i,sr=16000,hop_length=512, cmap="plasma")
        plt.xlabel("Time")
        plt.ylabel("Frequency")
        plt.colorbar(format='%+2.0f dB')
